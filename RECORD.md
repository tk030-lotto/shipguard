...
- **ワンクリック起動ランチャー (`shipguard-ui.bat`)**:
  ...
  - **Windows文字コード・構文仕様厳守**: `cmd.exe` において `if` ブロック内の括弧 `()` がコマンド終端として誤解釈される不具合（`'eq' は...` エラー）を解消するため、構文を極力シンプルにし括弧の誤認を完全に排除。改行コードは **CRLF (`0D 0A`)**、エンコーディングは **Shift-JIS (CP932)** で保存。末尾に `pause` を常設。
- **UIサーバーのポート自動再試行 (`src/commands/ui.ts`)**:
  - `EADDRINUSE`（ポート競合）検知時に、自動的に次の空きポート（3774, 3775...）へ最大10回フォールバックする耐障害ロジックを実装。
...
- **Windowsバッチファイル特有の落とし穴と知見**:
  - `cmd.exe` は改行コードが LF (`\n`) のみだと行区切りを正常に判定できず、さらに `if (...)` 内の括弧 `(v18以上)` がブロック終端として誤認される罠があるため、Windows用 `.bat` では「Shift-JIS (CP932)」「CRLF (`\r\n`)」「括弧ネストの排除」が重要であることを実証・反映。

---

## 2026-09-18: ファイル・フォルダ選択ダイアログの実装およびプロジェクト統計ツール準拠カラーへの刷新

### 1. 変更・実装内容
- **ファイル／フォルダのGUI選択機能 (`src/utils/dialog.ts`, `src/commands/ui.ts`)**:
  - OS標準の「フォルダーの参照」ダイアログ（`selectFolderDialog`）および「ファイルを開く」ダイアログ（`selectFileDialog`）を呼び出すAPI（`POST /api/browse-folder`, `POST /api/browse-file`）を実装。
  - 手入力不要で、UI上の「📁 フォルダ選択」「📄 ファイル選択」ボタンからマウスで直感的に対象を選択可能に。
  - `collectFiles` において単一ファイルが指定された場合の個別スキャンにもネイティブ対応。
- **プロジェクト統計ツール準拠のデザインシステム刷新 (`src/commands/ui-html.ts`, `src/reporter/html.ts`)**:
  - `knowledge/protocol.md`（第18条）のデザイン規約に完全統一。
  - `#09090b` (メイン背景), `#121215` (カード背景), `#27272a` (ボーダー), `#fafafa` (主要テキスト), `#a1a1aa` (補足テキスト)。
  - フォント: `Inter`, `JetBrains Mono` を導入し、機能的でシックなミニマル・ダークUIへ全面改修。
- **テスト・品質検証**:
  - `test/ui.test.ts` にダイアログAPIエンドポイントのテストを追加（全13テストファイル・46テストケース全件合格、型エラー0件、tsupビルド成功）。
  - 全ソースファイルで300行原則を徹底（最大218行）。

### 2. 技術的決定・背景
- **手入力ゼロ化による操作ミス撲滅**:
  - パスの手入力（タイポや相対パスの解決ミス）によるスキャン失敗を完全に排除するため、OSのネイティブダイアログをNode.jsから安全に呼び出して絶対パスを自動反映するアーキテクチャを採用。
- **統一されたブランド体験**:
